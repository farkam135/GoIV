...
public class OcrManualCalibrationActivity extends AppCompatActivity {
    ...
    private void setAllUnknownFieldValuesToZero() {
        ...
        if (sfr.pokemonNameArea == null) {
            sfr.pokemonNameArea = new ScanArea(0, 0, 100, 100);
        }
        if (sfr.pokemonTypeArea == null) {
            sfr.pokemonTypeArea = new ScanArea(0, 0, 100, 100);
        }
        if (sfr.pokemonGenderArea == null) {
            sfr.pokemonGenderArea = new ScanArea(0, 0, 100, 100);
        }
        if (sfr.candyNameArea == null) {
            sfr.candyNameArea = new ScanArea(0, 0, 100, 100);
        }
        if (sfr.pokemonHpArea == null) {
            sfr.pokemonHpArea = new ScanArea(0, 0, 100, 100);
        }
        if (sfr.pokemonCpArea == null) {
            sfr.pokemonCpArea = new ScanArea(0, 0, 100, 100);
        }
        if (sfr.pokemonCandyAmountArea == null) {
            sfr.pokemonCandyAmountArea = new ScanArea(0, 0, 100, 100);
        }
        if (sfr.pokemonEvolutionCostArea == null) {
            sfr.pokemonEvolutionCostArea = new ScanArea(0, 0, 100, 100);
        }
        if (sfr.pokemonPowerUpStardustCostArea == null) {
            sfr.pokemonPowerUpStardustCostArea = new ScanArea(0, 0, 100, 100);
        }
        if (sfr.pokemonPowerUpCandyCostArea == null) {
            sfr.pokemonPowerUpCandyCostArea = new ScanArea(0, 0, 100, 100);
        }

        if (sfr.arcCenter == null) {
            sfr.arcCenter = new ScanPoint(screenshot.getWidth() / 2, 100);
        }

        if (sfr.infoScreenCardWhitePixelPoint == null) {
            sfr.infoScreenCardWhitePixelPoint = new ScanPoint(0, 0);
        }
        if (sfr.infoScreenFabGreenPixelPoint == null) {
            sfr.infoScreenFabGreenPixelPoint = new ScanPoint(0, 0);
        }
    }
    ...
    private void addModifiableFieldsForCalibrationVariables() {
        ...
        addCalibrationView(sfr.pokemonTypeArea, "Type", "The type text, such as fire/wind");
        ...
        addCalibrationView(sfr.candyNameArea, "Candy name", "The text under the current candy amount number, such "
                + "as \"ABRA CANDY\". Make the area wide, some pokemon have really long names.");
    ...
    private void addArcCalibrationView(final ScanFieldResults sfr) {
        ...
        arcViewLayout.setBackgroundColor(Color.parseColor("#20000000"));
